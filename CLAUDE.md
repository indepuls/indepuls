...
- **Trouvé en retour bêta, non encore traité — `dateOuverture` du mode démo écrasée par un merge de params stale** : `loadData()` recharge `getExampleData()` à chaque rechargement du mode démo (`isExample:true`) puis fusionne `DATA.params = {...demoData.params, ...savedParams}` pour préserver les réglages déjà modifiés par l'utilisatrice — mais si `savedParams.dateOuverture` a été persisté vide (champ "Quand avez-vous lancé votre activité ?" optionnel, jamais rempli automatiquement dans le parcours démo), le spread écrase le `dateOuverture:'2026-03'` généré par `getExampleData()`. Conséquence observée : `getActiveMonthsInYear()` retombe à 12 (comportement par défaut sans date renseignée) au lieu de 10, et l'objectif annuel affiché (Paramètres › Mes objectifs, "CA mensuel à atteindre = X/an") comme la trajectoire annuelle repassent en ×12 non proratisé — alors que les missions démo elles-mêmes ne couvrent que mars→décembre. Le fix de proratisation (bullet ci-dessus) est mathématiquement correct mais silencieusement neutralisé dans ce scénario précis. **Pas corrigé dans ce chantier** — nécessite de décider (avec Faustine) si `dateOuverture` doit être forcé/reconstruit à chaque rechargement démo (comme le reste de `getExampleData()`) ou si le champ doit rester éditable/persistant même en mode démo. Question produit connexe en cours de discussion : faut-il simplifier le mode démo sur une année pleine (retire la proratisation de l'équation démo, mais retire aussi la couverture de test de ce cas réel), ou rendre visible quelque part que l'année est incomplète et que l'objectif est proratisé en conséquence.
